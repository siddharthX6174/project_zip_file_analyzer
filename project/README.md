...
- **Groq API** : Groq API for code & documentation analysis
...
4. **Configure Environment Variables**
   ...
   Copy [.env.example](.env.example) to [.env](.env) and set your `GROQ_API_KEY`.
...
1. **Start the Application**
   ...
   Run the FastAPI application from the `project` folder using Uvicorn:
...
- The archive should contain at least one source file and a `README.md` file.
- The app expects a `GROQ_API_KEY` in [.env](.env) at runtime.
